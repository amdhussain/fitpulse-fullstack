...
// async function bookClass(userId, { classId, bookingDate, bookingTime, notes, paymentMethod, paymentOption }) {
//   logger.info('Booking attempt', { userId, classId, bookingDate, bookingTime });
  
//   const cls = await BookingRepository.findClassById(classId);

//   if (!cls) {
//     logger.error('Class not found for booking', { classId, userId });
...
//   // Validate bookingTime matches class schedule
//   const schedule = cls.schedule ? JSON.parse(cls.schedule) : [];
//   const bookingTimeMatch = schedule.some(slot => slot.startTime === bookingTime || slot.endTime === bookingTime);
  
//   if (!bookingTimeMatch) {
//     logger.error('Booking time does not match class schedule', { classId, bookingTime, schedule });
//     throw new BadRequestError('Invalid schedule time selected. The selected time does not match the class schedule.');
//   }
...
//       { userId, classId, trainerId: cls.trainerId, bookingDate, bookingTime, notes, paymentStatus: 'PENDING_PAYMENT', paymentOption: paymentOption || 'FULL' },
//       session
//     );
...
// async function bookTrainer(userId, { trainerId, bookingDate, bookingTime, sessionType, notes, paymentMethod, paymentOption }) {
...
async function bookClass(userId, { classId, bookingDate, bookingTime, notes, paymentMethod, paymentOption }) {
  ...
  // শিডিউল চেকিং বা ভ্যালিডেশন শর্তটি এখান থেকে সম্পূর্ণ রিমুভ করে দেওয়া হয়েছে, 
  // ফলে এখন ইউজার যেকোনো সময় দিয়ে খুব সহজেই বুকিং করতে পারবে।
  ...
  const className = cls.name || 'a class';
  const dateTime = bookingDate && bookingTime ? `${bookingDate} at ${bookingTime}` : 'upcoming';
  notificationService.bookingCreated(booking.id, `User ${userId}`, className, dateTime, userId).catch(() => {});
...
async function bookTrainer(userId, { trainerId, bookingDate, bookingTime, sessionType, notes, paymentMethod, paymentOption }) {
  ...
  const trainerName = trainer.user ? `${trainer.user.firstName} ${trainer.user.lastName}` : 'trainer';
  const dateTime = bookingDate && bookingTime ? `${bookingDate} at ${bookingTime}` : 'upcoming';
  notificationService.bookingCreated(booking.id, `User ${userId}`, `session with ${trainerName}`, dateTime, userId).catch(() => {});
...
async function cancelBooking(userId, bookingId, cancelReason) {
  ...
  notificationService.bookingCancelled(bookingId, `User ${userId}`, 'session', userId).catch(() => {});
