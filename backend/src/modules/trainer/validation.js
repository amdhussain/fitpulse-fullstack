...
const createTrainer = validateRequest([
  ...
  rules.optionalPositiveInteger('sessionDuration'),
  ...
  body('availableTimeSlots').optional().isArray().withMessage('Available time slots must be an array'),
  body('availableTimeSlots.*').optional().trim().matches(/^([01]\d|2[0-3]):[0-5]\d$/).withMessage('Each time slot must be in HH:MM format'),
  body('sessionTypes').optional().isArray().withMessage('Session types must be an array'),
  body('sessionTypes.*').optional().trim().isLength({ min: 1, max: 100 }).withMessage('Each session type must be between 1 and 100 characters'),
  body('membershipCompatibility').optional().isArray().withMessage('Membership compatibility must be an array'),
  body('membershipCompatibility.*').optional().trim().isLength({ min: 1, max: 100 }).withMessage('Each membership type must be between 1 and 100 characters'),
