...
function CmsBadge({ status = "active", onToggle, label }) {
  const safeStatus = typeof status === "object" ? status.status || "active" : String(status).toLowerCase();
  const styles = statusStyles[safeStatus] || statusStyles.active;
  const displayLabel = typeof label === "object" ? label.status || safeStatus : (label || safeStatus);
