...
function FileUpload({ label, value, onChange, color = "blue" }) {
  return (
    <div>
      ...
      <label
        className={`flex items-center justify-center gap-2 px-4 py-8 rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-600 ${borderColors[color] || borderColors.blue} transition-colors cursor-pointer group hover:bg-gray-50 dark:hover:bg-gray-800`}
      >
        <FiUpload
          className={`w-5 h-5 text-gray-300 dark:text-gray-500 ${iconColors[color] || iconColors.blue} transition-colors`}
        />
