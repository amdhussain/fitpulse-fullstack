...
import { tools, calculateFFMI } from "../../lib/fitnessTools";
...
const tool = tools.find((t) => t.id === "ffmi");

function FFMICalculatorPage() {
  ...
  const handleCalculate = () => {
    ...
    setResult(calculateFFMI(parseFloat(weight), parseFloat(height), parseFloat(neck), parseFloat(waist), gender, unit));
  };
  ...
  return (
    <FitnessLayout tool={tool}>
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
        ...
        <Button onClick={handleCalculate} variant="primary" className="w-full">Calculate FFMI</Button>
      </div>
      {result && (
        <div className="grid grid-cols-2 gap-3 mt-4">
          <ResultCard value={result.value} label="FFMI Score" unit={result.unit} color="amber" />
          <ResultCard value={result.category} label="Category" unit="" color="purple" />
        </div>
      )}
    </FitnessLayout>
  );
