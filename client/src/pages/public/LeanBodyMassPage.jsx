...
import { tools, calculateLeanBodyMass } from "../../lib/fitnessTools";
...
const tool = tools.find((t) => t.id === "lean-body-mass");

function LeanBodyMassPage() {
  ...
  const handleCalculate = () => {
    ...
    setResult(calculateLeanBodyMass(parseFloat(weight), parseFloat(height), parseFloat(neck), parseFloat(waist), gender, unit));
  };
  ...
  return (
    <FitnessLayout tool={tool}>
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
        ...
        <Button onClick={handleCalculate} variant="primary" className="w-full">Calculate Lean Body Mass</Button>
      </div>
      {result && (
        <div className="grid grid-cols-2 gap-3 mt-4">
          <ResultCard value={result.value} label="Lean Body Mass" unit={result.unit} color="emerald" />
          <ResultCard value={`${result.bfPercent}%`} label="Body Fat" unit="%" color="red" />
        </div>
      )}
    </FitnessLayout>
  );
...
export default LeanBodyMassPage;
