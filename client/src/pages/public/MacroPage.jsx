...
import { tools, calculateMacros } from "../../lib/fitnessTools";
...
const tool = tools.find((t) => t.id === "macro");
...
function MacroPage() {
  ...
  const handleCalculate = () => {
    ...
    setResult(calculateMacros(parseFloat(weight), parseFloat(height), parseInt(age), gender, parseFloat(activity), goal, unit));
  };
  ...
  return (
    <FitnessLayout tool={tool}>
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
        ...
        <Button onClick={handleCalculate} variant="primary" className="w-full">Calculate Macros</Button>
      </div>
      {result && (
        <div className="grid grid-cols-2 gap-3 mt-4">
          <ResultCard value={result.calories} label="Daily Calories" unit="kcal" color="rose" />
          <ResultCard value={result.protein} label="Protein" unit="g/day" color="rose" />
          <ResultCard value={result.fat} label="Fat" unit="g/day" color="rose" />
          <ResultCard value={result.carbs} label="Carbs" unit="g/day" color="rose" />
        </div>
      )}
    </FitnessLayout>
  );
