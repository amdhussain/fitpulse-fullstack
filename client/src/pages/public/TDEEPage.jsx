...
import { tools, calculateTDEE } from "../../lib/fitnessTools";
...
const tool = tools.find((t) => t.id === "tdee");
...
function TDEEPage() {
  ...
  const handleCalculate = () => {
    ...
    setResult(calculateTDEE(parseFloat(weight), parseFloat(height), parseInt(age), gender, parseFloat(activity), unit));
  };
  ...
  return (
    <FitnessLayout tool={tool}>
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
        ...
        <Button onClick={handleCalculate} variant="primary" className="w-full">Calculate TDEE</Button>
      </div>
      {result && (
        <div className="grid grid-cols-2 gap-3 mt-4">
          <ResultCard value={result.bmr} label="BMR" unit="cal/day" color="amber" />
          <ResultCard value={result.value} label="TDEE" unit="cal/day" color="orange" />
        </div>
      )}
    </FitnessLayout>
  );
